...
if(isset($_POST["jid"])){
	...
	header("location: offerDetailsReceipt.php");
}
if(isset($_POST["jidfarminput"])){
	$_SESSION["offer_id"]=$_POST["jidfarminput"];
	// header("location: marketRequestinfo.php");
	header("location: farmInputofferDetails.php");
...
<!--client Profile Details-->	
		<div class="card" style="padding:20px 20px 5px 20px;margin:20px 0px 100px">
                  ...
                  <table style="width:100%">
                      ...
                      if ($result->num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo '
                                <form action="farmerProfile2.php" method="post">
			...
			<div class="panel panel-primary">
			  <div class="panel-heading">Accepted offers & Sold Maize Deals</div>
			  <div class="panel-body"><h4>
                  <table style="width:100%">
                      <tr>
                          <td style="font-weight:bold; padding-bottom:10px;">Crop Id</td>
			...
			<div class="panel panel-primary">
			  ...
			  <div class="panel-body"><h4>
                  <table style="width:100%">
                      ...
                      <?php 
                      	// $sql = "select * from `farm_output` left join `selected` on selected.offer_id=farm_output.offer_id where selected.f_username='$username'";
                      	$sql = "select * from `market_request` left join `selected` on selected.offer_id=market_request.id where selected.e_username='$username' order by timestamp desc";
						$result = $conn->query($sql);
                      if ($result->num_rows > 0) {
                            // output data of each row
                            while($row = $result->fetch_assoc()) {
            ...
            <div class="panel panel-primary">
			  ...
			  <div class="panel-body"><h4>
                  <table style="width:100%">
                      ...
                      <?php 
                      	$sql = "select * from `farm_input` left join `selected_farminput` on selected_farminput.offer_id=farm_input.item_id where selected_farminput.f_username='$username' order by timestamp desc";
						$result = $conn->query($sql);
                      if ($result->num_rows > 0) {
                            // output data of each row
                            while($row = $result->fetch_assoc()) {
                                ...
                                echo '
                                <form action="farmerProfile2.php" method="post">
                                <input type="hidden" name="jidfarminput" value="'.$offer_id.'">
                                    <tr>
                                    <td>'.$offer_id.'</td>
                                    <td><input type="submit" class="btn btn-link btn-lg" value="'.$title.'"></td>
                                    <td>'.$timestamp.'</td>
