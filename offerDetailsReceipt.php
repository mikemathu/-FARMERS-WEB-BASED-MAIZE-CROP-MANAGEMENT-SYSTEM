...
if(isset($_POST["f_hire"])){
    ...
    if($result==true){
		...
		$result = $conn->query($sql);
		if($result==true){
			$sql = "UPDATE market_request SET valid=0 WHERE offer_id='$offer_id'";
			$result = $conn->query($sql);
			if($result==true){
...
function generate_bill(){
	...
	// $query=mysqli_query($con,"select * from `farm_output` left join `accepted` on accepted.offer_id=farm_output.offer_id where farm_output.offer_id='$offer_id'  ");
	$query=mysqli_query($con,"select * from `market_request` left join `selected` on selected.offer_id=market_request.id where market_request.id='$offer_id'  ");
	...
	while($row = mysqli_fetch_array($query)){
	  ...
	  <label> <b>Price Per Bag:</b> Ksh.</label>'.$row["budget"].'<br/><br/>
...
<div id="applicant" class="card" style="padding:20px 20px 5px 20px;margin-top:20px">
			<div class="panel panel-success">
			  ...
			  <div class="panel-body"><h4>
                  ...
                  <tr>
                      <td style="font-weight: bold; font-style:italic;">Farmer's username
                    </td>
